...
第 4 步會依序做：確認服務擁有者 → 檢查系統需求 → 設時區與語系 →
...
未驗證過的備份等於沒有備份。這一步會把備份還原到一個暫時的資料庫
並比對筆數，不會動到正式資料。
...
curl localhost:3000/api/version    # {"appVersion":"0.19.0"} 現在跑的是哪一版
...
sudo install -d -m 0750 -o "$USER" -g "$USER" /var/backups/yunzhi
