 ...
 * 另一邊就這樣安靜地破一個洞。
 *
 * # 那麼個申落點模擬的機率怎麼辦（`lib/placement.mjs`）
 *
 * 落點模擬的輸出**就是機率**，而這一層擋掉所有機率形式的輸出。兩者
 * 不衝突，因為它們管的是**兩條不同的通道**：
 *
 *   · **確定性通道**（`lib/placement.mjs` → 落點那一頁）。數字是算出來
 *     的：輸入有快照（`SimulationRun.input`）、亂數有固定種子，所以
 *     任何一個數字都可以被重算出一模一樣的值。它與自己的資料基礎
 *     （用了哪幾年、什麼來源、可靠度分數、查詢日期）以及
 *     `STAGE_TWO_NOTE` 綁在同一個物件裡，而那幾樣東西是計算的一部分，
 *     不是文案——所以它們不可能在改版時被順手拿掉。
 *
 *   · **生成通道**（AI 老師，`lib/admissionRefDb.ts` 的 `adviceFor`）。
 *     數字是**寫出來的**。模型沒有辦法保證它寫的那個數字是剛剛算出來
 *     的那一個，而一個看起來一樣的數字讀者分不出來。
 *
 * 規則因此是：**機率只走確定性通道，AI 的文字裡永遠不出現機率。**
 *
 * 落實的方式**不是**在這裡開一個例外，而是在來源端就不把機率交給模型
 * ——`placement.mjs` 的 `placementAdvicePayload()` 送出的脈絡裡沒有任何
 * 機率欄位（有測試逐欄釘著）。
 *
 * 而且就算有人日後試圖走「把模擬算出來的數字加進 `adviceFacts().numbers`
 * 白名單」那條路，它也**通不過**：`ODDS_PREDICTION` 那一條擋的是
 * 「機率詞 ＋ 數字」這個**句型**，與那個數字有沒有來源無關。
 * `tests/placement.test.mjs` 有一條測試就是在斷言這件事。
 *
 * 那不是這一層的缺陷，是它的設計。要看機率就去看那張表，那裡的每一個
