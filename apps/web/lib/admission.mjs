 ...
 * **這一支回的不是「下一場學測是哪一年的」。**
 *
 * 學年度自 8 月起算而學測在 1 月，所以每年 1/20 到 7/31 之間，這一支
 * 回的那個學年度的學測**已經考完了**。志願、參考資料、繁星名單都以
 * 學年度為單位（那是簡章的單位），拿它們來說是對的；但級分預測的目標
 * 是「下一場還沒考的考試」，那要用 `lib/predict.mjs` 的
 * `upcomingGsatYear()`。
 *
 * 這裡不改成「下一場考試」的語意，理由是改了會動到志願與參考資料：
 * 一位學生在 3 月填的個申志願屬於 114 學年度，若這一支在 3 月回 115，
 * 他填完之後畫面上會顯示「你還沒有填志願」。**兩種語意都要存在，
 * 而它們必須是兩支函式。**
 */
...
// §8.5 每年要有人重新查一次的數字
//
// 這個模組裡**寫死的統計值集中在這一節**，而且每一項都帶著「它是哪
// 一年的」。散在各處的話，過期的方式是最壞的一種：那句話讀起來完全
// 正常，只是那個年份與那個數字都停在三年前，而畫面上沒有任何症狀。
//
// 制度規則（一校一名、每個位置 2 名、學群 1 至 8、放棄期限四天）不算
// 在這裡——那些多年不變，而且改的時候是整套制度改。這一節只收
// **逐年公告的統計量**。
// ═════════════════════════════════════════════════════════════════

/**
 * 繁星推薦全國缺額。**這是本模組唯一一個寫死的統計值。**
 *
 * 它的用途是反駁一個系統性的悲觀偏誤：把第二輪當成零。推薦序 2 的
 * 學生若以為自己完全沒機會，他會放棄一條真的存在的路——而缺額多到
 * 值得講出一個數字。
 *
 * **要用它就要連 `year` 一起顯示**（`starVacancySentence()` 就是為了
 * 這件事），因為隔年的缺額是另一個數字，而讀者分不出畫面上這個是哪
 * 一年的。維護的方式是每年繁星放榜後回來改這兩個欄位，一處改完全站
 * 都對——包含 `lib/adviceGuard.mjs` 的數字白名單。
 */
export const STAR_VACANCY_FACT = Object.freeze({
  /** 民國學年度。 */
  year: 115,
  /** 全國缺額（名）。 */
  count: 922,
...
/**
 * 把那個數字寫成一句**不會因為年份過去而變成謊話**的話。
 *
 * @param {number} [year] 現在在談的學年度。與那個數字同一年時直接陳述；
 *   不同年時明說「最近一次有數字的是哪一年」——而不是把舊數字掛上新
 *   年份，那是這一類文案最常見也最難發現的錯誤。
 */
export function starVacancySentence(year = STAR_VACANCY_FACT.year) {
  const f = STAR_VACANCY_FACT;
  if (Number(year) === f.year) {
    return `${f.year} 學年度繁星推薦全國缺額 ${f.count} 名`;
  }
  return (
    `最近一次有數字的是 ${f.year} 學年度：繁星推薦全國缺額 ${f.count} 名` +
    `（${year} 學年度的缺額要去當年度的公告查，每年都不一樣）`
  );
}
